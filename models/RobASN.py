...
class RobASN(nn.Module):
    def __init__(self, args, transition_system, vocab, io_vocab):
        ...
        # init
        module_input_size = 2 * args.io_hid_size + 2 * args.enc_hid_size + args.enc_hid_size
        comp_type_modules = []
        ...
        # init
        cnstr_type_modules = []
        cnstr_input_size = 2 * args.enc_hid_size + args.field_emb_size
        for prod in grammar.productions:
            cnstr_type_modules.append((prod.constructor.name,
                                       ConstructorTypeModule(args, prod, cnstr_input_size)))
    ...
    def _score(self, ex):
        ...
        return self._score_node(self.grammar.root_type, init_state, ex.tgt_actions, context_vecs,  io_context_vecs)
    ...
    def _score_node(self, node_type, v_state, action_node, context_vecs, io_context_vecs):
        ...
        if node_type.is_primitive_type():
            ...
            # scores = mask * module()
            scores = module.score(v_output, contexts, io_contexts)
        ...
        scores = comp_module.score(v_output, contexts, io_contexts)
        score = -1 * scores.view([-1])[action_node.action.choice_index]
        ...
        for next_field, next_state, next_action in zip(cnstr.fields, cnstr_results, action_node.fields):
            score += self._score_node(next_field.type, next_state, next_action, context_vecs, io_context_vecs)
        return score
    ...
    def naive_parse(self, ex):
        ...
        action_tree = self._naive_parse(self.grammar.root_type, init_state, context_vecs, io_context_vecs, 1)

        return self.transition_system.build_ast_from_actions(action_tree)

    def _naive_parse(self, node_type, v_state, context_vecs, io_context_vecs, depth):
        ...
        if node_type.is_primitive_type():
            module = self.prim_type_dict[node_type.name]
            # scores = mask * module()
            scores = module.score(v_state[0], contexts, io_contexts).cpu().numpy().flatten()
            # scores =  [tgt_action_tree.action.choice_idx]
            # b * choice
            # score = -1 * scores.view([-1])[action_node.action.choice_index]
        ...
        comp_module = self.comp_type_dict[node_type.name]
        scores = comp_module.score(v_state[0], contexts, io_contexts).cpu().numpy().flatten()
        choice_idx = np.argmax(scores)
        ...
        # next_states = self.v_lstm( [1 * 1 * x], v_state)
        cnstr_results = cnstr_module.update(self.v_lstm, v_state, contexts)
        action_fields = [self._naive_parse(next_field.type, next_state, context_vecs, io_context_vecs, depth+1) for next_field, next_state in zip(cnstr.fields, cnstr_results)]
    ...
    def precompute_scores_for_holes(self, hyp, context_vecs, io_context_vecs):
        # v_state = v_state.torch.unsqueeze(0)
        ...
        for pending_node in nodes_to_proc:
            ...
            if node_type.is_primitive_type():
                module = self.prim_type_dict[node_type.name]
                # scores = mask * module()
                scores = module.score(v_state[0], contexts, io_contexts)
                scores = scores.squeeze(0)
                pending_node.action.cache_scores(scores, module.vocab, smooth_factor=smooth_factor)
            else:
                comp_module = self.comp_type_dict[node_type.name]
                scores = comp_module.score(v_state[0], contexts, io_contexts)
                scores = scores.squeeze(0)
    ...
    def ex_guided_parse_and_track_budget(self, ex, cache=None):
        ...
        self.precompute_scores_for_holes(init_hyp, context_vecs, io_context_vecs)
        cur_beam = [init_hyp]
        num_executed = 0
        ...
        for ts in range(self.args.max_decode_step):
            hyp_pools = []
            for hyp in cur_beam:
                continuations = self.continuations_of_hyp_with_cache(hyp, context_vecs, io_context_vecs)
            ...
            for hyp_i, hyp  in enumerate(hyp_pools[:num_slots]):
                ...
                if hyp.is_complete():
                    ...
                    self.precompute_scores_for_holes(hyp, context_vecs, io_context_vecs)
                    cur_beam.append(hyp)
    ...
    def continuations_of_hyp_with_cache(self, hyp, context_vecs, io_context_vecs):        
    ...
    def initialize_hyp_for_synthesizing(self, ex):
        ...
        init_hyp = Hypothesis.init_hypothesis(self.grammar.root_type, init_state, order=self.args.search_order)
        self.precompute_scores_for_holes(init_hyp, context_vecs, io_context_vecs)
        return init_hyp, (context_vecs, io_context_vecs)
